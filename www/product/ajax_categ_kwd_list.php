...
$kwd_categ	= trim($_REQUEST['kwd_categ']);		// T:상의, B:하의, O:아웃웨어, U:언더웨어, A:ACC
$kwd_kind	= trim($_REQUEST['kwd_kind']);		// S:스타일별, I:아이템별, T:테마별
$page		= trim($_REQUEST['page']);			// 페이지
$kwd		= trim($_REQUEST['kwd']);			// 키워드
$order		= trim($_REQUEST['order']);			// A:최근등록순서, B:평가마감순서, C:랭킹순서
$s_key		= trim($_REQUEST['s_key']);			// S:샵이름, P:코디이름, A:샵+코디제목, K:키워드
$keyword	= trim($_REQUEST['keyword']);		// ajax 입력 검색어
