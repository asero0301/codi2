 ...
 * file   : /coditop/manage/product/ing_list.php
 * date   : 2008.08.18
 * desc   : Admin ing list
...
$cond = " where 1 and A.p_idx = B.p_idx and (now() between B.start_dt and B.end_dt) ";
if ( $key != "" ) {
...
/*
select A.*, B.shop_name, C.start_dt, C.end_dt,
(select ifnull(sum(X.score),0) from tblScoreConfig X, tblScore Y where X.sc_cid = Y.sc_cid and Y.p_e_idx = C.p_e_idx) as score_sum
from tblProduct A,tblShop B,tblProductEach C 
where 1 
AND A.shop_idx = B.shop_idx 
AND A.p_idx = C.p_idx 
AND now() BETWEEN C.start_dt AND C.end_dt
ORDER BY A.p_reg_dt DESC limit 0, 20
*/
$cond = "";
$cond = " where 1 AND A.shop_idx = B.shop_idx AND A.p_idx = C.p_idx AND ( now() BETWEEN C.start_dt AND C.end_dt ) ";
$orderby = " ORDER BY A.p_reg_dt DESC ";
if ( $key != "" ) {
	$cond .= " and $key like '%$kwd%' ";
}
	...
	$LIST .= "
		<tr>
		  ...
		  <td align='center' bgcolor='#FFFFFF'class='stext'>
			<input type='button' value='추천등록' $act_flag onClick='chgJudge(\"Y\",\"$p_idx\");'>
