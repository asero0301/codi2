...
//	$mem_percent = $row['lg_percent'];
	...
	$_SESSION['mem_id'] = $s_mem_id;
	$_SESSION['mem_name'] = $s_mem_name;
	...
	if ( $mem_kind == "U" ) {	// 일반회원용
		...
		$arr_user_info = getMyUserInfo($s_mem_id);
		...
		$arr_shop_info = getMyShopInfo($s_mem_id);
		...
		// 대표샵 이름을 구한다.
		$sql = "select shop_name from tblShop where mem_id = '$id' and shop_kind = 'I' ";


		$s_shop_name = $mainconn->count($sql);

		$_SESSION['shop_name'] = $s_shop_name;
	...
	if ( $idsave == "Y" ) {
		setCookie("idsave", $s_mem_id, time()+86400*30, "/", "coditop10.com");
	}
